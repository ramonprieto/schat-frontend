...
export default {
  ...
  methods: {
    ...
    connectToWebSocket() {
      const roomName = this.$route.params.uri
      console.log('Attempting to connect to websocket at .','ws://127.0.0.1:8000/ws/chat/' + roomName + '/');
      this.chatSocket = new ReconnectingWebSocket(
        'ws://127.0.0.1:8000/ws/chat/' + roomName + '/'
      );
      console.log('Successfully connected to websocket.');
      this.chatSocket.onopen = this.onOpen;
      this.chatSocket.onclose = this.onClose;
      this.chatSocket.onmessage = this.onMessage;
      this.chatSocket.onerror = this.onError;
    },

    onOpen(event) {
      console.log('Connectioned opened.', event.data);
    },
    ...
    onError(event) {
      alert('An error ocurred: ', event.data)
    }
  }
}
</script>
